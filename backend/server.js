...
const executionLimiter = rateLimit({
  ...
  message: {
    error:
      "Rate limit exceeded. You can only execute code 15 times per minute.",
  },
});
...
const aiLimiter = rateLimit({
  ...
  message: {
    error:
      "AI limit exceeded. You can query the AI assistant 10 times per minute.",
  },
});
...
app.get("/api/problems", async (req, res) => {
  try {
    ...
    // Map them for clean JSON deliveriesasdasd
    res.json(rows);
  } catch (err) {
    res
      .status(500)
      .json({ error: `Failed to retrieve problems: ${err.message}` });
  }
});
...
app.get("/api/problems/:id", async (req, res) => {
  try {
    const row = await dbGet(
      "SELECT id, title, description, difficulty, starter_code, function_name, param_names FROM problems WHERE id = ?",
      [req.params.id],
    );
    if (!row) {
    ...
    res
      .status(500)
      .json({ error: `Failed to retrieve problem: ${err.message}` });
  }
});
...
app.post("/api/problems", authenticateToken, async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        error:
          "All fields (title, description, difficulty, functionName, paramNames, starterCode, testCases) are required.",
      });
  }
  ...
  try {
    ...
    const paramsStr =
      typeof paramNames === "string" ? paramNames : JSON.stringify(paramNames);
    ...
    res
      .status(201)
      .json({ message: "Coding challenge published dynamically!" });
  } catch (err) {
    res
      .status(500)
      .json({ error: `Failed to create challenge: ${err.message}` });
  }
});
...
app.post("/api/execute", executionLimiter, async (req, res) => {
  ...
  if (!code || !language) {
    return res
      .status(400)
      .json({ error: "Code content and programming language are required." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `Sandboxed execution failed: ${err.message}` });
  }
});
  ...
  async (req, res) => {
    ...
    if (!problemId || !code || !language) {
      return res
        ...
        .json({
          error:
            "Problem ID, code content, and programming language are required.",
        });
    }

    try {
      // Retrieve target problem structure
      const problem = await dbGet("SELECT * FROM problems WHERE id = ?", [
        problemId,
      ]);
      if (!problem) {
      ...
      res
        .status(500)
        .json({ error: `Submission evaluation crashed: ${err.message}` });
    }
  },
);
...
app.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Failed to construct leaderboard: ${err.message}` });
  }
});
...
app.post("/api/ai/explain", aiLimiter, async (req, res) => {
  ...
  if (!code || !language) {
    return res
      ...
      .json({
        error:
          "Code contents and language are required to prompt the AI Tutor.",
      });
  }

  try {
    const aiExplanation = await explainError(
      code,
      language,
      error,
      stdout,
    ...
    res
      .status(500)
      .json({ error: `AI Tutor failed to analyze: ${err.message}` });
  }
});
...
app.post("/api/ai/complexity", aiLimiter, async (req, res) => {
  ...
  if (!code || !language) {
    return res
      .status(400)
      .json({ error: "Code content and language are required." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `AI Complexity analyst failed: ${err.message}` });
  }
});
...
app.post("/api/ai/chat", aiLimiter, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: `AI Copilot failed to reply: ${err.message}` });
  }
});
...
initDb()
  .then(() => {
    app.listen(PORT, () => {
      console.log(`====================================================`);
      console.log(
        `🚀 CodeRank Backend API is live on: http://localhost:${PORT}`,
      );
      console.log(`====================================================`);
    });
  })
